function [profile_data] = simulate( params, material, method, scene, profile_info )
...
while T > 0
  ...
  while t > 0
    ...
    switch lower(params.integration)
      %--------------------------------------------------------------------
      case 'implicit_amalie'
        
        [state, conv] = method.implicit_step_amalie(dh, mesh, state, params, material, BC, profile,method);
        
      %------------------------------------------------------------------
      case 'implicit'
        [state, conv] = method.implicit_step(dh, mesh, state, params, material, BC, profile);
      ...
      case 'adaptive'
        ...
        s2 = method.semi_implicit_step( dh, mesh, params, s1, BC, false );
        
        s3 = method.semi_implicit_step( dh/2, mesh, params, s1, BC, false );
        
        s3 = method.compute_elastic_forces(mesh, s3, params);
        s4 = method.semi_implicit_step( dh/2, mesh, params, s3, BC, false );
      ...
      case 'fixed'
        ...
        [state, conv ]= method.semi_implicit_step( dh, mesh, params, state, BC, profile );
...
%     fh = figure('Visible','off');  % Should be used if run in commandline mode
    ...
    grid minor
    view(3)
    title( ['T = '  num2str( params.T - T ) ' [s]' ' : ' num2str(params.material)] );
    xlabel('x [m]');
    ylabel('y [m]');
    zlabel('z [m]');
    hold off;
    ...
    if profile_info.save_images
      if(frame<=max_frames)
        filename = strcat(  profile_info.output_path,  profile_info.filename_prefix, num2str( frame, '%.5u'), num2str(params.material,'%.5u')  );
        for f=1:length(profile_info.image_formats)
          format = profile_info.image_formats{f};
          print(fh, filename, format);
        end        
        frame = frame + 1;
      end
    end
...
% filename_test_vectors = strcat(scene.name, num2str(scene.mesh_no));
% save(filename_test_vectors,'test_vectors');
