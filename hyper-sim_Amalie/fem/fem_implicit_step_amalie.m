...
%      v = v0 - tau*nabla_psi;
     ...
     b = fem_phi( coords,  dt, mesh, state, params, material, bcon);
     psi_tau = b'*b;
     if(psi_tau < psi)
...
%     dist(det(nabla_phi))
    ...
    tau = 0.2;
    order = '1st';
    final_new = @(xx) afunAx(xx, order, dt, state, mesh, params, material, tau);
    
    tol = 1e-3;
    maxit = 500;
    
    [delta,flag,~] =  pcg(final_new, -phi,tol,maxit);
...
% %       disp('hej')
%       p_tau   = p + delta*tau;
%       v_tau   = v + 1/dt * delta*tau;
%       x_tau   = [p_tau; v_tau];
%       phi_tau = fem_phi( x_tau,  dt, mesh, state, params, material, bcon);
