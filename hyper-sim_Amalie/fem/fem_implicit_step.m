function [ state, conv, flag, iter, message ] = fem_implicit_step_amalie(dt, mesh, state, params, material, bcon, profile, iter_max, abs_tol, rel_tol, dir_tol, stag_tol)
...
%   display('Gradient descent method...');
  for i=1:0,
    ...
    %  x         = x - nabla_psi
   b = fem_phi( x,  dt, mesh, state, params, material, bcon);  
   J = fem_nabla_phi( x,  0, mesh, state, params, material, bcon);
   nabla_psi = J' * b;
   ...
   while 1,
     x = x0 - tau*nabla_psi;
%      phis = fem_phi( x,  dt, mesh, state, params, bcon);
%      b = -dt*phis(length(x)/2+1:end) - (M+ dt*C) * phis(1:length(x)/2);
...
%       disp(iter)
    ...
    phi = fem_phi( x,  dt, mesh, state, params, material, bcon);
    ...
    %--- Compute Jacobian -------------------------------------------------
    nabla_phi = fem_nabla_phi( x,  dt, mesh, state, params, material, bcon);
    
    %--- Compute Newton Direction -----------------------------------------
    tau = 1;
    order = '1st';
    final_new = @(xx) afunAx(xx, order, dt, state, free, mesh, params, material, k, tau, Mff, M, C);

    [x1,fl1,rr1,it1,rv1] = gmres( final_new, -phi);
    [x2,fl2,rr2,it2,rv2] = pcg(final_new, -phi);
    [x3,fl3,rr3,it3,rv3] = pcg(final_new, -phi,[],[],M);
    
    
  
    figure;
    semilogy(0:it3,rv3/norm(-phi),'b.');
    hold on;
    semilogy(0:it1,rv1/norm(-phi),'r.');
    semilogy(0:it2,rv2/norm(-phi),'k.');
    legend('pcg w. preconditioner','gmres','pcg w/o');
